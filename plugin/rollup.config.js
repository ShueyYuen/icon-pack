      ...
      userScriptHeader({
        ...
        transformHeaderContent(items) {
          const newItems = [
            ...items
              .map(([name, value]) => {
                switch (name) {
                  ...
                  case '@version':
                    return [name, `${value}.${Math.floor(new Date().getTime() / 1000)}`]
                  default:
                    return [name, value]
                }
              }),
            [
              '@include',
              `${process.env.ICON_PLUGIN_BACKEND}/*`
            ],
            [
              "@downloadURL",
              `${process.env.ICON_PLUGIN_BACKEND}/main.user.dev.js`
            ]
          ];
...
function prodConfigs() {
  const outputFile = 'dist/main.user.js'
  return defineConfig({
    input: 'src/main.ts',
    output: {
      file: outputFile,
      ...
      userScriptHeader({
        ...
        transformHeaderContent(items) {
          const newItems = [
            ...
            [
              "@downloadURL",
              `${process.env.ICON_PLUGIN_BACKEND}/main.user.js`
            ]
          ];
          return newItems
        },
